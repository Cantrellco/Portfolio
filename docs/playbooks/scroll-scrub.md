...
> Source: Higgsfield website-builder bundle, fetched 2026-07-26. Kept as a technique reference for the seam-locked video-journey pattern (A4). Our site is custom Vite+R3F, not the Higgsfield builder — the media-chain + encode + runtime rules below are the transferable part.
...
Use the existing generation tools, not an external CLI:

1. Call `models_explore` for the current image/video model schemas. Require the
   exact media roles the chosen architecture needs; do not rely on a remembered
   model roster or invent `start_image`/`end_image` roles.
2. For A, generate only the entry-still candidates in ONE batched
   `generate_image` call. For B, batch all independent scene stills in one
   call. Reuse the locked style preamble verbatim, name concrete scene props,
   request no text/logos/watermarks, and keep the focal point centered.
3. Poll the returned jobs with `job_display` because their outputs are
   downstream inputs. Keep the approved full-size source stills in scratch
   space. They are generation inputs/art direction, not runtime posters;
   deployed posters come from the exact encoded clips below.
...
5. Poll and download every source MP4. Load
   `references/scroll-scrub-asset-video.md` with
   `get_website_creation_bundle_file`, copy its fenced Bash into
   ...
   `bounds` writes `<prefix>-first.png` and `<prefix>-last.png`. Upload each
   boundary frame with `media_upload`, PUT the bytes to its presigned URL, and
   call `media_confirm` before using its media id in `generate_video`. The
   public poster commands run AFTER encoding, so each `poster`/`mobilePoster`
   matches the first frame of the exact clip the browser will decode.
...
Load both bundled code assets with `get_website_creation_bundle_file` and
adapt them:
